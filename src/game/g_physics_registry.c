...
void TC_EntPhysicsRegister(int entNum, const char* shader, const float mins[3], const float maxs[3]){
    ...
    g_reg[i].used=1; g_reg[i].entNum=entNum; g_reg[i].haveAABB=0; g_reg[i].origin[0]=g_reg[i].origin[1]=g_reg[i].origin[2]=0.0f;
    ...
    else { float pm[3], pM[3]; if (TC_Phys_ParseFile(g_reg[i].shader, pm, pM)==0){ for(int k=0;k<3;++k){ g_reg[i].mins[k]=pm[k]; g_reg[i].maxs[k]=pM[k]; } g_reg[i].haveAABB=1; } else { fallback_aabb(g_reg[i].mins, g_reg[i].maxs); } }
